...
```
代码这样子写的目的是根据不同功能的实时性要求，分配不同的执行频率，兼顾功能稳定性和系统效率
...
```
为什么这里缓冲区第一个放0呢：Nixie_Buf[0] = 0 这个值本身没有任何实际作用，之所以这么写，是和 “数组下标对应数码管物理位置” 的设计逻辑匹配
- 这里要提一嘴，这里使用的是动态数码管闲事，由于数码管一次只能显示一位，所以要一下子显示多位则要利用人的视觉暂留效应，通过循环使其不断切换从而让我们觉得又一次显示多位的错觉，这有点像CPU的多任务切换，所以这就是设置缓冲区的目的，方便后续显示使用
